...
@StockTestcase(StockTestcaseName.F10_STOCKBULLETINLISTTEST_1)
public class F10_StockBulletinListTest_1 {
    private static final StockTestcaseName testcaseName = StockTestcaseName.F10_STOCKBULLETINLISTTEST_1;
    private static SetupConfig.TestcaseConfig testcaseConfig;
    private static final int timeout_ms = 1000000;
    @BeforeClass
    public static void setup() throws Exception {
        Log.d("F10_StockBulletinListTest_1", "Setup");
    ...
    public void requestWork() throws Exception {
        Log.d("F10_StockBulletinListTest_1", "requestWork");
...
//        for (int i=0;i<quoteNumbers.length;i++){
        ...
        if(quoteNumbers2.equals("null")){
            ...
            request.sendV2(quoteNumbers,Integer.parseInt(quoteNumbers1),newsID,quoteNumbers3,new IResponseInfoCallback<StockBulletinListResponse>() {
                @Override
                public void callback(StockBulletinListResponse stockBulletinListResponse) {
                    ...
                    try {
                        if (stockBulletinListResponse.list!=null){
                            for (int i=0;i<stockBulletinListResponse.list.size();i++) {
                                JSONObject uploadObj_1 = new JSONObject();
                                uploadObj_1.put("PUBDATE_", stockBulletinListResponse.list.get(i).PUBDATE_);
                                uploadObj_1.put("ID_", stockBulletinListResponse.list.get(i).ID_);
                                uploadObj_1.put("TITLE_", stockBulletinListResponse.list.get(i).TITLE_);
                                uploadObj_1.put("dataSource", stockBulletinListResponse.list.get(i).dataSource);
                                uploadObj_1.put("STOCKNAME_", stockBulletinListResponse.list.get(i).STOCKNAME_);
