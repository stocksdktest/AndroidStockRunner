...
public class ChartV2Test_6 {
    ...
    public void requestWork() throws Exception {
        ...
        final String []isNeedAfterHours = rule.getParam().getString("isNeedAfterHours").split(",");
        ...
        for (int i=0;i<quoteNumbers.length;i++){
            ...
            quoteDetailRequest.send(quoteNumbers[i], new IResponseInfoCallback() {
                ...
                public void callback(Response response) {
                    ...
                    request.send(quoteItem,Types[b], new IResponseInfoCallback() {
                        @Override
                        public void callback(Response response) {
...
//                    uploadObj.put
                            ...
                            for (OHLCItem item : chartResponse.historyItems) {
                                Log.d("StockUnittest", quoteNumbers[b]+item.datetime);
                            }
                    ...
                    }, Boolean.parseBoolean(isNeedAfterHours[b]));
                }
                @Override
                public void exception(ErrorInfo errorInfo) {
                    result.completeExceptionally(new Exception(errorInfo.toString()));
