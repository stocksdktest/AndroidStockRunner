...
//                        uploadObj.put("changeRate", list.upDownFlag+list.changeRate);//ios注意
                        ...
                        //ios无
                        ArrayList<JSONObject> tradeTickList=new ArrayList<>();
                        if (list.tradeTick!=null&&list.tradeTick.length>0){
                            for (int j=0;j<list.tradeTick.length;j++){
                                JSONObject uploadObj_2 = new JSONObject();
                                uploadObj_2.put("type",list.tradeTick[j][0] == null ? "-" : list.tradeTick[j][0]);
                                uploadObj_2.put("time",list.tradeTick[j][1] == null ? "-" : list.tradeTick[j][1]);
                                uploadObj_2.put("tradeVolume",list.tradeTick[j][2] == null ? "-" : list.tradeTick[j][2]);
                                uploadObj_2.put("tradePrice",list.tradeTick[j][3] == null ? "-" : list.tradeTick[j][3]);
                                tradeTickList.add(uploadObj_2);
                            }
                            uploadObj.put("tradeTick",new JSONArray(tradeTickList));
                        }
