...
import com.mitake.core.keys.quote.PlateIndexCustomField;
...
public class PlateIndexQuoteTest_2 {
    ...
    public void requestWork() throws Exception {
        Log.d("PlateIndexQuoteTest_2", "requestWork");
        // TODO get custom args from param

        final String quoteNumbers = rule.getParam().optString("CODES", "");
        final String[] customFiled = rule.getParam().optString("COSTOMFILEDS", "").split(",");
        final CompletableFuture result = new CompletableFuture<JSONObject>();
//        for (int i=0;i<quoteNumbers.length;i++){
        int[] custom=new int[customFiled.length];
        if (customFiled[0].equals("null")){
            custom=null;
        }else {
            for (int i=0;i<customFiled.length;i++){
                custom[i]=Integer.parseInt(customFiled[i]);
            }
        }
//        PlateIndexCustomField
            PlateIndexQuoteRequest request = new PlateIndexQuoteRequest();
            request.send(quoteNumbers,custom, new IResponseInfoCallback<PlateIndexResponse>() {
                @Override
